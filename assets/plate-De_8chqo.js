import { e as ut, f as Dt, k as Ft, o as vt, x as mt, v as W, g as At, j as Nt, b as Rt, d as Tt } from "./styles-pdtxawzU.js";
import { a as It } from "./analyze-CmnHHflB.js";
import { d as Ot, __tla as __tla_0 } from "./deformCpp-CS-wCYO-.js";
import { g as Pt } from "./getParameters-Dl3mTgVW.js";
import { g as _t } from "./getDialog-SKblNfmB.js";
import { g as nt } from "./getReport-9byhZbfH.js";
import { g as jt, __tla as __tla_1 } from "./getMesh-DmUdekin.js";
import { o as ht } from "./unsafe-html-CJlcZ_0W.js";
...
`;
    for (let p = 0; p < t.length; p++) {
      ...
      for (let v = 0; v < t[p].length; v++) n += `<span class="td">${j(t[p][v], a)}</span>`;
...
`;
  ...
  function Bt(t, a, n, p) {
    const u = 110 / Math.max(t, 1), z = (120 - 2 * 25) / Math.max(a, 1), L = Math.min(u, z), P = t * L, $ = a * L, E = 25, S = 25;
    let x = '<svg viewBox="0 0 160 120" xmlns="http://www.w3.org/2000/svg" style="width:160pt; height:120pt; font-family:Arial; font-size:9px;">';
    x += `<defs>
  ...
  </defs>`, x += `<rect x="${E}" y="${S}" width="${P}" height="${$}" fill="#e8e8e8" stroke="#333" stroke-width="1.5"/>`;
    const g = 4, R = 5;
    for (let m = 0; m <= R; m++) {
      const c = E + P * m / R;
      x += `<polygon points="${c},${S + $} ${c - g},${S + $ + g} ${c + g},${S + $ + g}" fill="#999" stroke="#666" stroke-width="0.5"/>`;
    }
    for (let m = 0; m <= R; m++) {
      const c = S + $ * m / R;
      x += `<polygon points="${E},${c} ${E - g},${c - g} ${E - g},${c + g}" fill="#999" stroke="#666" stroke-width="0.5"/>`;
    }
    for (let m = 0; m <= R; m++) {
      const c = S + $ * m / R;
      x += `<polygon points="${E + P},${c} ${E + P + g},${c - g} ${E + P + g},${c + g}" fill="#999" stroke="#666" stroke-width="0.5"/>`;
    }
    for (let m = 0; m <= R; m++) {
      const c = E + P * m / R;
      x += `<polygon points="${c},${S} ${c - g},${S - g} ${c + g},${S - g}" fill="#999" stroke="#666" stroke-width="0.5"/>`;
    ...
    const M = 3;
    for (let m = 1; m <= M; m++) for (let c = 1; c <= M; c++) {
      const C = E + P * m / (M + 1), B = S + $ * c / (M + 1);
      x += `<line x1="${C}" y1="${B - 8}" x2="${C}" y2="${B + 5}" stroke="#0066cc" stroke-width="1.2" marker-end="url(#arrowEnd)"/>`;
    }
    x += `<text x="${E + P / 2}" y="${S + $ / 2 - 12}" text-anchor="middle" fill="#0066cc" font-style="italic">q</text>`;
    const X = S + $ + 18;
    x += `<line x1="${E}" y1="${X}" x2="${E + P}" y2="${X}" stroke="#333" stroke-width="0.8" marker-start="url(#arrowStart)" marker-end="url(#arrowEnd)"/>`, x += `<text x="${E + P / 2}" y="${X + 10}" text-anchor="middle" font-style="italic">a</text>`;
    const f = E + P + 18;
    return x += `<line x1="${f}" y1="${S}" x2="${f}" y2="${S + $}" stroke="#333" stroke-width="0.8" marker-start="url(#arrowStart)" marker-end="url(#arrowEnd)"/>`, x += `<text x="${f + 8}" y="${S + $ / 2 + 3}" font-style="italic">b</text>`, x += "</svg>", x;
  }
  function Yt(t, a, n = 400) {
    if (!t.length || !a.length) return "<p>No mesh data</p>";
    const p = t.map((f) => f[0]), v = t.map((f) => f[1]), b = Math.min(...p), h = Math.max(...p), u = Math.min(...v), z = Math.max(...v), L = h - b || 1, P = z - u || 1, $ = n / Math.max(L, P), E = P * $, S = 20, x = 2.666667, g = 0.01, R = /* @__PURE__ */ new Set();
    t.forEach((f, m) => {
      (Math.abs(f[0] - b) < g || Math.abs(f[0] - h) < g || Math.abs(f[1] - u) < g || Math.abs(f[1] - z) < g) && R.add(m);
    });
    let M = `<svg viewbox="-20 -20 ${O(n + 2 * S, 0)} ${O(E + 2 * S, 6)}" xmlns="http://www.w3.org/2000/svg" version="1.1" style=" font-family: Segoe UI; font-size:10px; width:${n}pt; height:${O(E, 6)}pt">`;
    M += "<style>.joint{fill:orangeRed;} .support{stroke:red; stroke-width:1; fill:lightpink;} .element{stroke:seaGreen; stroke-width:1; fill:lime; fill-opacity:0.1; stroke-opacity:0.5}</style>", M += `<rect x="0" y="0" width="${n}" height="${O(E, 6)}" style="fill:yellow; fill-opacity:0.2" />`, a.forEach((f, m) => {
      const c = f.reduce((d, y) => d + t[y][0], 0) / f.length, C = f.reduce((d, y) => d + t[y][1], 0) / f.length, B = (c - b) * $, G = E - (C - u) * $, r = f.map((d) => {
        const y = (t[d][0] - b) * $, k = E - (t[d][1] - u) * $;
        return `${O(y, 6)},${O(k, 6)}`;
      }).join(" ");
      M += `<text x="${O(B, 6)}" y="${O(G + 4, 6)}" text-anchor="middle">${m + 1}</text>`, M += `<polygon points="${r}" class="element"/>`;
    });
    const X = 5.333333;
    return R.forEach((f) => {
      const m = (t[f][0] - b) * $, c = E - (t[f][1] - u) * $;
      (Math.abs(t[f][1] - u) < g || Math.abs(t[f][1] - z) < g) && (M += `<line x1="${O(m - 2 * X, 6)}" y1="${O(c, 6)}" x2="${O(m + 2 * X, 6)}" y2="${O(c, 6)}" class="support"/>`), (Math.abs(t[f][0] - b) < g || Math.abs(t[f][0] - h) < g) && (M += `<line x1="${O(m, 6)}" y1="${O(c - 2 * X, 6)}" x2="${O(m, 6)}" y2="${O(c + 2 * X, 6)}" class="support"/>`), M += `<circle cx="${O(m, 6)}" cy="${O(c, 6)}" r="${X}" class="support"/>`;
    }), t.forEach((f, m) => {
      const c = (f[0] - b) * $, C = E - (f[1] - u) * $;
      M += `<circle cx="${O(c, 6)}" cy="${O(C, 6)}" r="${x}" class="joint" />`, M += `<text x="${O(c + x * 2, 6)}" y="${O(C - x, 6)}" text-anchor="start">${m + 1}</text>`;
    }), M += "</svg>", M;
  }
  function Gt(t, a, n, p = 400, v = "w") {
    if (!t.length || !a.length || !n.length) return "";
    const b = t.map((r) => r[0]), h = t.map((r) => r[1]), u = Math.min(...b), z = Math.max(...b), L = Math.min(...h), P = Math.max(...h), $ = z - u || 1, E = P - L || 1, S = p / Math.max($, E), x = E * S, g = 20, R = n.filter((r) => r !== void 0 && !isNaN(r));
    if (!R.length) return "";
    const M = Math.min(...R), X = Math.max(...R), f = Math.abs(X - M) || 1;
    function m(r) {
      const d = Math.max(0, Math.min(1, (r - M) / f));
      let y, k, D;
      return d < 0.25 ? (y = 0, k = Math.floor(255 * d * 4), D = 255) : d < 0.5 ? (y = 0, k = 255, D = Math.floor(255 * (1 - (d - 0.25) * 4))) : d < 0.75 ? (y = Math.floor(255 * (d - 0.5) * 4), k = 255, D = 0) : (y = 255, k = Math.floor(255 * (1 - (d - 0.75) * 4)), D = 0), `rgb(${y},${k},${D})`;
    }
    const c = `colorScale_${v.replace(/[^a-zA-Z0-9]/g, "")}`;
    let C = `<svg viewBox="-20 -20 ${p + 80} ${x + 2 * g}" xmlns="http://www.w3.org/2000/svg" style="width:${p + 80}pt; height:${O(x, 0)}pt; font-family:Arial; font-size:9px;">
<defs>
...
<stop offset="100%" style="stop-color:#ff0000"/>
    ...
    a.forEach((r) => {
      const d = r.reduce((D, F) => D + (n[F] || 0), 0) / r.length, y = m(d), k = r.map((D) => {
        const F = (t[D][0] - u) * S, T = x - (t[D][1] - L) * S;
        return `${O(F, 6)},${O(T, 6)}`;
      }).join(" ");
      C += `<polygon points="${k}" fill="${y}" stroke="#333" stroke-width="0.3"/>`;
    });
    const B = p + 10, G = x - 20;
    C += `<rect x="${B}" y="10" width="15" height="${O(G, 0)}" fill="url(#${c})" stroke="#333"/>`;
    for (let r = 0; r <= 5; r++) {
      const d = M + f * r / 5, y = 10 + G - G * r / 5;
      C += `<text x="${B + 20}" y="${O(y + 3, 0)}">${j(d, 4)}</text>`;
    }
    return C += `<text x="${B}" y="5">${v}</text>`, C += "</svg>", C;
  }
  function Mt(t, a, n, p = 400, v = "M") {
    if (!t.length || !a.length) return "";
    const b = n.filter((r) => r !== void 0 && !isNaN(r));
    if (!b.length) return "<p><i>No data for contour map</i></p>";
    const h = t.map((r) => r[0]), u = t.map((r) => r[1]), z = Math.min(...h), L = Math.max(...h), P = Math.min(...u), $ = Math.max(...u), E = L - z || 1, S = $ - P || 1, x = p / Math.max(E, S), g = S * x, R = 20, M = Math.min(...b), X = Math.max(...b), f = Math.abs(X - M) || 1;
    function m(r) {
      const d = Math.max(0, Math.min(1, (r - M) / f));
      let y, k, D;
      return d < 0.25 ? (y = 0, k = Math.floor(255 * d * 4), D = 255) : d < 0.5 ? (y = 0, k = 255, D = Math.floor(255 * (1 - (d - 0.25) * 4))) : d < 0.75 ? (y = Math.floor(255 * (d - 0.5) * 4), k = 255, D = 0) : (y = 255, k = Math.floor(255 * (1 - (d - 0.75) * 4)), D = 0), `rgb(${y},${k},${D})`;
    }
    const c = `colorScaleElem_${v.replace(/[^a-zA-Z0-9]/g, "")}`;
    let C = `<svg viewBox="-20 -20 ${p + 80} ${g + 2 * R}" xmlns="http://www.w3.org/2000/svg" style="width:${p + 80}pt; height:${O(g, 0)}pt; font-family:Arial; font-size:9px;">
<defs>
<linearGradient id="${c}" x1="0%" y1="100%" x2="0%" y2="0%">
<stop offset="0%" style="stop-color:#0000ff"/>
<stop offset="25%" style="stop-color:#00ffff"/>
...
<stop offset="100%" style="stop-color:#ff0000"/>
    ...
    a.forEach((r, d) => {
      const y = n[d] || 0, k = m(y), D = r.map((F) => {
        const T = (t[F][0] - z) * x, K = g - (t[F][1] - P) * x;
        return `${O(T, 6)},${O(K, 6)}`;
      }).join(" ");
      C += `<polygon points="${D}" fill="${k}" stroke="#333" stroke-width="0.3"/>`;
    });
    const B = p + 10, G = g - 20;
    C += `<rect x="${B}" y="10" width="15" height="${O(G, 0)}" fill="url(#${c})" stroke="#333"/>`;
    for (let r = 0; r <= 5; r++) {
  ...
  function $t(t) {
    ...
    const a = ((_a = t == null ? void 0 : t.nodes) == null ? void 0 : _a.val) || [], n = ((_b = t == null ? void 0 : t.elements) == null ? void 0 : _b.val) || [], p = ((_c = t == null ? void 0 : t.nodeInputs) == null ? void 0 : _c.val) || {}, v = ((_d = t == null ? void 0 : t.elementInputs) == null ? void 0 : _d.val) || {}, b = ((_e = t == null ? void 0 : t.deformOutputs) == null ? void 0 : _e.val) || {}, h = ((_f = t == null ? void 0 : t.analyzeOutputs) == null ? void 0 : _f.val) || {}, u = ((_g = v.elasticities) == null ? void 0 : _g.values().next().value) || 100, z = ((_h = v.poissonsRatios) == null ? void 0 : _h.values().next().value) || 0.3, L = ((_i = v.thicknesses) == null ? void 0 : _i.values().next().value) || 1, P = ((_k = (_j = p.loads) == null ? void 0 : _j.values().next().value) == null ? void 0 : _k[2]) || -3, $ = a.map((A) => A[0]), E = a.map((A) => A[1]), S = $.length ? Math.max(...$) - Math.min(...$) : 15, x = E.length ? Math.max(...E) - Math.min(...E) : 10, g = [];
    let R = 0, M = 0;
    b.deformations && b.deformations.forEach((A, N) => {
      g[N] = A[2] || 0, Math.abs(A[2]) > Math.abs(R) && (R = A[2], M = N);
    });
    const X = [], f = [], m = [], c = [], C = [], B = [];
    let G = 0, r = 0, d = 0, y = 0;
    h.bendingXX && h.bendingXX.forEach((A, N) => {
      const s = A.reduce((I, q) => I + q, 0) / A.length;
      X[N] = s, Math.abs(s) > Math.abs(G) && (G = s, r = N);
    }), h.bendingYY && h.bendingYY.forEach((A, N) => {
      const s = A.reduce((I, q) => I + q, 0) / A.length;
      f[N] = s, Math.abs(s) > Math.abs(d) && (d = s, y = N);
    ...
    const k = u * Math.pow(L, 3) / (12 * (1 - z * z)), D = [
      [
        k,
        k * z,
        0
    ...
    ], F = ((_l = p.supports) == null ? void 0 : _l.size) || 0, T = a.length, K = n.length, at = `
<style>${Lt}
/* Zoom control styles */
.zoom-toolbar {
  position: sticky;
...
<h4>Input data</h4>
<table><tr><td>
<p>Plate dimensions - ${Y(`${l("a")} = ${j(S, 1)}`)} ${ut()}, ${Y(`${l("b")} = ${j(x, 1)}`)} ${ut()}</p>
<p>Thickness - ${Y(`${l("t")} = ${j(L, 2)}`)} ${ut()}</p>
<p>Load per node - ${Y(`${l("P")} = ${j(Math.abs(P), 1)}`)} ${Dt()}</p>
<p>Modulus of elasticity - ${Y(`${l("E")} = ${j(u, 0)}`)} ${Ft()}</p>
<p>Poisson's ratio - ${Y(`${l("\u03BD")} = ${j(z, 2)}`)}</p>
</td><td style="vertical-align:middle; padding-left:30px;">
${Bt(S, x)}
</td></tr></table>

<h4>Finite element mesh</h4>
<p>We will use triangular finite elements with ${Y(`${l("n")} = 18`)} DOFs (6 per node)</p>
<p>Total number of elements - ${Y(`${l("n", "e")} = ${K}`)}</p>
<p>Total number of joints - ${Y(`${l("n", "j")} = ${T}`)}</p>
<p>Supported joints count - ${Y(`${l("n", "s")} = ${F}`)}</p>
...
`).join("")}
${T > 10 ? `<tr><td colspan="3">... (${T - 10} more joints)</td></tr>` : ""}
</table>
...
<p>${Y(`${l("N", "1")}(\u03BE,\u03B7) = 1 - \u03BE - \u03B7`)}</p>
<p>${Y(`${l("N", "2")}(\u03BE,\u03B7) = \u03BE`)}</p>
<p>${Y(`${l("N", "3")}(\u03BE,\u03B7) = \u03B7`)}</p>

<p><b>Constitutive matrix</b> (stress - strain relationship)</p>
<p>${Y(`<b>${l("D")}</b> = ${it(`${l("E")} \xB7 ${l("t")}<sup>3</sup>`, `12 \xB7 (1 - ${l("\u03BD")}<sup>2</sup>)`)} \xB7 <b class="b1">[</b>1; ${l("\u03BD")}; 0 <b class="b0">|</b> ${l("\u03BD")}; 1; 0 <b class="b0">|</b> 0; 0; ${it(`1 - ${l("\u03BD")}`, "2")}<b class="b1">]</b>`)}</p>
<p>${Y(`<b>${l("D")}</b> = ${it(`${j(u, 0)} \xB7 ${j(L, 2)}<sup>3</sup>`, `12 \xB7 (1 - ${j(z, 2)}<sup>2</sup>)`)} \xB7 <b class="b1">[</b>1; ${j(z, 2)}; 0 <b class="b0">|</b> ${j(z, 2)}; 1; 0 <b class="b0">|</b> 0; 0; ${it(`1 - ${j(z, 2)}`, "2")}<b class="b1">]</b> = ${Xt(D, 6)}`)}</p>

<p><b>Strain-displacement matrix</b></p>
<p>The bending strain-displacement matrix ${Y(`${l("B", "b")}`)} (3\xD718) relates curvatures to nodal DOFs:</p>
<p>${Y(`\u03BA = {${l("\u03BA", "xx")}, ${l("\u03BA", "yy")}, ${l("\u03BA", "xy")}}<sup>T</sup> = ${l("B", "b")} \xB7 ${l("u")}`)}</p>

<p><b>Element stiffness matrix</b></p>
<p>${Y(`${l("K", "e")} = \u222B\u222B ${l("B")}<sup>T</sup> \xB7 ${l("D")} \xB7 ${l("B")} dA`)}</p>
<p>For shell elements: ${Y(`${l("K", "e")} = ${l("K", "b")} + ${l("K", "s")} + ${l("K", "m")}`)}</p>
<p>(bending + shear + membrane)</p>

<h4>Solution</h4>
<p>Total degrees of freedom: ${Y(`${T * 6}`)}</p>
<p>Free degrees of freedom: ${Y(`${T * 6 - F * 6}`)}</p>
<p>Global system: ${Y(`${l("K")} \xB7 ${l("U")} = ${l("F")}`)}</p>
<p>Solution method: LU decomposition</p>

<h4>Results</h4>
<p><b>Displacements</b></p>
<p>Maximum displacement: ${Y(`${l("w", "max")} = <span class="${Math.abs(R) > 10 ? "err" : "ok"}">${j(R, 6)}</span>`)} at joint ${M + 1}</p>
...
`).join("") : ""}
${(((_m = b.deformations) == null ? void 0 : _m.size) || 0) > 10 ? `<tr><td colspan="4">... (${(((_n = b.deformations) == null ? void 0 : _n.size) || 0) - 10} more)</td></tr>` : ""}
</table>

<p><b>Displacement contour map</b></p>
${Gt(a, n, g, 400, "w")}
<p>${Y(`${l("w")}(${it(l("a"), "2")}, ${it(l("b"), "2")}) = ${j(R, 6)}`)}</p>

<h4>Shell Results</h4>
<p><b>Bending moments</b></p>
<p>Maximum bending moment ${l("M", "x")}: ${Y(`${l("M", "x,max")} = ${j(G, 4)}`)} ${vt()} at element ${r + 1}</p>
<p>Maximum bending moment ${l("M", "y")}: ${Y(`${l("M", "y,max")} = ${j(d, 4)}`)} ${vt()} at element ${y + 1}</p>
...
<tr><td>${N + 1}</td><td>${j(X[N] || 0, 4)}</td><td>${j(f[N] || 0, 4)}</td><td>${j(m[N] || 0, 4)}</td></tr>
`).join("")}
...
${n.slice(0, 10).map((A, N) => `
<tr><td>${N + 1}</td><td>${j(c[N] || 0, 4)}</td><td>${j(C[N] || 0, 4)}</td><td>${j(B[N] || 0, 4)}</td></tr>
`).join("")}
${K > 10 ? `<tr><td colspan="4">... (${K - 10} more elements)</td></tr>` : ""}
</table>
` : ""}
...
`;
    for (let p = 0; p < t.length; p++) {
      n += '<span class="tr">';
      for (let v = 0; v < t[p].length; v++) {
        const b = t[p][v], h = w(b, 4);
...
`;
  ...
  function wt(t, a) {
    ...
    let n = a ? `<b>${a}</b> = ` : "";
    n += '<span class="vector">';
    for (let p = 0; p < t.length; p++) n += `<span class="td">${w(t[p], 6)}</span>`;
    return n += "</span>", n;
  }
  function Vt(t) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k;
    const a = ((_a = t == null ? void 0 : t.nodes) == null ? void 0 : _a.val) || [], n = ((_b = t == null ? void 0 : t.elements) == null ? void 0 : _b.val) || [], p = ((_c = t == null ? void 0 : t.nodeInputs) == null ? void 0 : _c.val) || {}, v = ((_d = t == null ? void 0 : t.elementInputs) == null ? void 0 : _d.val) || {}, b = ((_e = t == null ? void 0 : t.deformOutputs) == null ? void 0 : _e.val) || {}, h = ((_f = t == null ? void 0 : t.analyzeOutputs) == null ? void 0 : _f.val) || {}, u = ((_g = v.elasticities) == null ? void 0 : _g.values().next().value) || 21e4, z = ((_h = v.poissonsRatios) == null ? void 0 : _h.values().next().value) || 0.3, L = ((_i = v.thicknesses) == null ? void 0 : _i.values().next().value) || 0.2, P = u / (2 * (1 + z)), $ = u * Math.pow(L, 3) / (12 * (1 - z * z)), E = 5 / 6, S = a.map((s) => s[0]), x = a.map((s) => s[1]), g = S.length ? Math.max(...S) - Math.min(...S) : 10, R = x.length ? Math.max(...x) - Math.min(...x) : 15, M = a.length, X = n.length, f = ((_j = p.supports) == null ? void 0 : _j.size) || 0, m = M * 6;
    let c = -10;
    if (((_k = p.loads) == null ? void 0 : _k.size) > 0) {
      const s = p.loads.values().next().value;
      s && (c = s[2] || -10);
    }
    let C = 0;
    const B = [];
    b.deformations && b.deformations.forEach((s, I) => {
      B[I] = s, Math.abs(s[2]) > Math.abs(C) && (C = s[2]);
    });
    let G = 0;
    h.bendingXX && h.bendingXX.forEach((s) => {
      const I = s.reduce((q, rt) => q + rt, 0) / s.length;
    ...
    if (n.length > 0 && a.length >= 3) {
      ...
      const gt = rt[0], yt = rt[1], kt = bt[0], Et = bt[1], Ct = ft[0], St = ft[1];
      D = Math.abs((kt - gt) * (St - yt) - (Ct - gt) * (Et - yt)) / 2, F = [];
      for (let J = 0; J < 9; J++) {
        F[J] = [];
        for (let Q = 0; Q < 9; Q++) J === Q ? F[J][Q] = $ * D * (1 + J % 3 * 0.1) : Math.abs(J - Q) <= 3 ? F[J][Q] = $ * D * z * 0.3 * ((J + Q) % 2 ? -1 : 1) : F[J][Q] = 0;
      }
    }
    const T = Array(Math.min(12, M * 6)).fill(0);
    for (let s = 0; s < Math.min(4, M); s++) T[s * 3 + 2] = c;
    const K = [];
    for (let s = 0; s < Math.min(4, M); s++) {
      const I = B[s] || [
        0,
        0,
        0,
...
<h4>Geometr\xEDa:</h4>
<p>${_(`${i("B")} = <b>${w(g, 2)}</b>`)}&nbsp;&nbsp;(ancho en direcci\xF3n X)</p>
<p>${_(`${i("L")} = <b>${w(R, 2)}</b>`)}&nbsp;&nbsp;(largo en direcci\xF3n Y)</p>
<p>${_(`${i("t")} = <b>${w(L, 4)}</b>`)}&nbsp;&nbsp;(espesor de la placa)</p>

<h4>Material:</h4>
<p>${_(`${i("E")} = <b>${w(u, 0)}</b>`)}&nbsp;&nbsp;(m\xF3dulo de elasticidad)</p>
<p>${_(`${i("\u03BD")} = <b>${w(z, 2)}</b>`)}&nbsp;&nbsp;(coeficiente de Poisson)</p>

<h4>Carga:</h4>
<p>${_(`${i("q")} = <b>${w(c, 2)}</b>`)}&nbsp;&nbsp;(carga por nodo en direcci\xF3n Z)</p>
...
<p>${_(`${i("G")} = ${ot(i("E"), `2\xB7(1 + ${i("\u03BD")})`)} = ${ot(w(u, 0), `2\xB7(1 + ${w(z, 2)})`)} = <b>${w(P, 2)}</b>`)}</p>

<h4>Rigidez a flexi\xF3n:</h4>
<p>${_(`${i("D")} = ${ot(`${i("E")}\xB7${i("t")}<sup>3</sup>`, `12\xB7(1 - ${i("\u03BD")}<sup>2</sup>)`)} = ${ot(`${w(u, 0)}\xB7(${w(L, 4)})<sup>3</sup>`, `12\xB7(1 - ${w(z, 2)}<sup>2</sup>)`)} = <b>${w($, 6)}</b>`)}</p>

<h4>Factor de correcci\xF3n de corte (Mindlin-Reissner):</h4>
<p>${_(`${i("\u03BA")} = ${ot("5", "6")} = <b>${w(E, 4)}</b>`)}</p>

<h4>Rigidez a corte:</h4>
<p>${_(`${i("D", "s")} = ${i("\u03BA")}\xB7${i("G")}\xB7${i("t")} = ${w(E, 4)}\xB7${w(P, 2)}\xB7${w(L, 4)} = <b>${w(d, 4)}</b>`)}</p>
...
<div class="code-block"><span class="kw">const</span> { nodes, elements, boundaryIndices } = <span class="fn">getMesh</span>({
  points: [
    ...
    [<span class="num">${w(g, 1)}</span>, <span class="num">0</span>, <span class="num">0</span>],      <span class="cm">// Nodo 1: esquina X</span>
    [<span class="num">${w(g, 1)}</span>, <span class="num">${w(R, 1)}</span>, <span class="num">0</span>],   <span class="cm">// Nodo 2: esquina XY</span>
    [<span class="num">0</span>, <span class="num">${w(R, 1)}</span>, <span class="num">0</span>],      <span class="cm">// Nodo 3: esquina Y</span>
  ],
...
<p>${_(`${i("n", "j")} = <b>${M}</b>`)}&nbsp;&nbsp;(n\xFAmero de nodos)</p>
<p>${_(`${i("n", "e")} = <b>${X}</b>`)}&nbsp;&nbsp;(n\xFAmero de elementos triangulares)</p>
<p>${_(`${i("n", "s")} = <b>${f}</b>`)}&nbsp;&nbsp;(nodos con apoyo en el borde)</p>

<h4>Tabla de Nodos (primeros 6):</h4>
<table class="bordered">
  <tr><th>Nodo</th><th>X</th><th>Y</th><th>Z</th></tr>
  ${at.map((s) => `<tr><td>${s.i}</td><td>${w(s.x, 3)}</td><td>${w(s.y, 3)}</td><td>${w(s.z, 3)}</td></tr>`).join("")}
  ${M > 6 ? `<tr><td colspan="4" style="text-align:center">... (${M - 6} nodos m\xE1s)</td></tr>` : ""}
</table>
...
<ul>
  <li>D<sub>11</sub> = D<sub>22</sub> = ${w($, 4)} \u2192 Rigidez a flexi\xF3n pura</li>
  <li>D<sub>12</sub> = D<sub>21</sub> = ${w($ * z, 4)} \u2192 Acoplamiento por Poisson</li>
  <li>D<sub>33</sub> = ${w($ * (1 - z) / 2, 4)} \u2192 Rigidez a torsi\xF3n</li>
</ul>
</div>
...
<p>Coordenadas:</p>
<ul>
  <li>P1 = (${w(k.coords[0][0], 3)}, ${w(k.coords[0][1], 3)})</li>
  <li>P2 = (${w(k.coords[1][0], 3)}, ${w(k.coords[1][1], 3)})</li>
  <li>P3 = (${w(k.coords[2][0], 3)}, ${w(k.coords[2][1], 3)})</li>
</ul>
...
<p>${_(`[${i("K")}]_{global} \u2208 \u211D^{${m} \xD7 ${m}}`)}&nbsp;&nbsp;(${m} = ${M} nodos \xD7 6 DOF)</p>
...
<p>Cada nodo recibe carga ${_(`${i("F", "z")} = <b>${w(c, 2)}</b>`)} en direcci\xF3n Z.</p>
...
<p style="font-size: 14pt;">${_(`${i("w", "max")} = <b>${w(C, 6)}</b>`)}</p>
</div>

<div class="success">
<h4>\u2705 Momento m\xE1ximo M<sub>xx</sub>:</h4>
<p style="font-size: 14pt;">${_(`${i("M", "xx,max")} = <b>${w(G, 4)}</b>`)}</p>
</div>

<h4>Tabla de desplazamientos (primeros 6 nodos):</h4>
<table class="bordered">
  <tr><th>Nodo</th><th>u<sub>x</sub></th><th>u<sub>y</sub></th><th>u<sub>z</sub></th></tr>
  ${at.map((s) => `<tr><td>${s.i}</td><td>${w(s.ux, 6)}</td><td>${w(s.uy, 6)}</td><td class="${Math.abs(s.uz) > 1e-3 ? "err" : ""}">${w(s.uz, 6)}</td></tr>`).join("")}
</table>
...
`;
    for (let p = 0; p < t.length; p++) {
      n += '<span class="tr"><span class="td"></span>';
      for (let v = 0; v < t[p].length; v++) {
        const b = t[p][v], h = typeof b == "number" ? Kt(b, a) : b;
        n += `<span class="td">${h}</span>`;
      }
      n += `<span class="td"></span></span>
`;
...
<p>${o(`<b>${e("B", "pb")}</b> = ${V("1", "2A")} \xB7 `)}${pt(v)}</p>
...
<p>${o(`<b>${e("C")}</b> = ${V(e("E"), `1 - ${e("\u03BD")}<sup>2</sup>`)} \xB7 `)}${pt(p)}</p>
...
`;
  ...
  function zt(t, a, n, p, v) {
    if (!t.length || !a.length || !n.length) return `<p style="color:#999;margin:10px;">No data available for ${v}</p>`;
    const b = t.map((r) => r[0]), h = t.map((r) => r[1]), u = Math.min(...b), z = Math.max(...b), L = Math.min(...h), P = Math.max(...h), $ = z - u || 1, E = P - L || 1, S = p / Math.max($, E), x = E * S, g = 20, R = n.filter((r) => r !== void 0 && !isNaN(r));
    if (!R.length) return "";
    const M = Math.min(...R), X = Math.max(...R), f = Math.abs(X - M) || 1;
    function m(r) {
    ...
    const c = `grad_${v.replace(/[^a-zA-Z0-9]/g, "")}`;
    let C = `<svg viewBox="-20 -20 ${p + 80} ${x + 2 * g}" xmlns="http://www.w3.org/2000/svg" style="width:${p + 80}px;height:${x + 40}px;font-family:Arial;font-size:10px;">
<defs>
<linearGradient id="${c}" x1="0%" y1="100%" x2="0%" y2="0%">
<stop offset="0%" style="stop-color:#0000ff"/>
<stop offset="25%" style="stop-color:#00ffff"/>
...
<stop offset="100%" style="stop-color:#ff0000"/>
    ...
    a.forEach((r) => {
      const d = r.reduce((D, F) => D + (n[F] || 0), 0) / r.length, y = m(d), k = r.map((D) => {
        const F = (t[D][0] - u) * S, T = x - (t[D][1] - L) * S;
        return `${F.toFixed(2)},${T.toFixed(2)}`;
      }).join(" ");
      C += `<polygon points="${k}" fill="${y}" stroke="#333" stroke-width="0.3"/>`;
    });
    const B = p + 10, G = Math.max(x - 20, 60);
    C += `<rect x="${B}" y="10" width="15" height="${G}" fill="url(#${c})" stroke="#333"/>`;
    for (let r = 0; r <= 4; r++) {
      const d = M + f * r / 4, y = 10 + G - G * r / 4;
      C += `<text x="${B + 20}" y="${y + 4}">${d.toExponential(2)}</text>`;
    }
    return C += `<text x="${B}" y="5" font-weight="bold">${v}</text>`, C += "</svg>", C;
  }
  function Wt(t) {
    var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m, _n, _o, _p, _q, _r, _s;
    const a = ((_a = t == null ? void 0 : t.nodes) == null ? void 0 : _a.val) || [], n = ((_b = t == null ? void 0 : t.elements) == null ? void 0 : _b.val) || [], p = ((_c = t == null ? void 0 : t.nodeInputs) == null ? void 0 : _c.val) || {}, v = ((_d = t == null ? void 0 : t.elementInputs) == null ? void 0 : _d.val) || {}, b = ((_e = t == null ? void 0 : t.deformOutputs) == null ? void 0 : _e.val) || {}, h = ((_f = t == null ? void 0 : t.analyzeOutputs) == null ? void 0 : _f.val) || {}, u = (/* @__PURE__ */ new Date()).toLocaleDateString("en-US", {
      day: "numeric",
      month: "long",
      year: "numeric"
    }), z = ((_j = (_h = (_g = v.elasticities) == null ? void 0 : _g.values) == null ? void 0 : (_i = _h.call(_g)).next) == null ? void 0 : _j.call(_i).value) || 21e4, L = ((_n = (_l = (_k = v.poissonsRatios) == null ? void 0 : _k.values) == null ? void 0 : (_m = _l.call(_k)).next) == null ? void 0 : _n.call(_m).value) || 0.3, P = ((_r = (_p = (_o = v.thicknesses) == null ? void 0 : _o.values) == null ? void 0 : (_q = _p.call(_o)).next) == null ? void 0 : _r.call(_q).value) || 0.2, $ = ((_s = p.supports) == null ? void 0 : _s.size) || 0, E = z * Math.pow(P, 3) / (12 * (1 - L * L)), S = z / (2 * (1 + L)), x = b.deformations || [], g = [];
    let R = 0, M = 0;
    x.forEach((F, T) => {
      g[T] = F[2] || 0, Math.abs(F[2]) > Math.abs(R) && (R = F[2], M = T);
    });
    const X = h.bendingXX || [], f = h.bendingYY || [], m = [];
    let c = 0, C = 0;
    n.forEach((F, T) => {
      const K = X[T] || [
        0
        ...
        0
      ], A = K.reduce((s, I) => s + I, 0) / K.length, N = at.reduce((s, I) => s + I, 0) / at.length;
      m[T] = A, Math.abs(A) > Math.abs(c) && (c = A), Math.abs(N) > Math.abs(C) && (C = N);
    });
    let B = "";
    for (let F = 0; F < Math.min(a.length, 8); F++) {
      const T = a[F];
      B += `<tr><td>${F}</td><td>${(T[0] || 0).toFixed(3)}</td><td>${(T[1] || 0).toFixed(3)}</td><td>${(T[2] || 0).toFixed(3)}</td></tr>`;
    }
    let G = "";
    for (let F = 0; F < Math.min(x.length, 8); F++) {
      const T = x[F] || [
        0,
        0,
        0,
      ...
      G += `<tr><td>${F}</td><td>${U(T[0] || 0)}</td><td>${U(T[1] || 0)}</td><td>${U(T[2] || 0)}</td></tr>`;
    }
    const r = zt(a, n, g, 320, "w [displacement]"), d = new Array(a.length).fill(0), y = new Array(a.length).fill(0);
    n.forEach((F, T) => {
      F.forEach((K) => {
        d[K] += m[T] || 0, y[K]++;
      });
    ...
    const k = zt(a, n, d, 320, "Mxx [bending]"), D = `
<style>${Ht}</style>
...
<div class="formula-section">
  ...
  <div class="eq">
    ${H("E")} = ${tt(U(z))} <span class="unit">(elastic modulus)</span>
  </div>
  <div class="eq">
    ${H("\u03BD")} = ${tt(U(L, 2))} <span class="unit">(Poisson's ratio)</span>
  </div>
  <div class="eq">
...
<div class="formula-section">
  ...
  <div class="eq">
    ...
    = ${ct(tt(U(z)) + " \xB7 " + tt(U(P)) + "\xB3", "12 \xB7 (1 - " + tt(U(L, 2)) + "\xB2)")}
    = ${lt(U(E))}
  </div>
...
<div class="formula-section">
  ...
  <div class="eq">
    ${H("w")}${H("<sub>max</sub>")} = ${lt(U(R))} <span class="unit">at node ${M}</span>
  </div>

  <h3>Maximum Bending Moments</h3>
  <div class="eq">
    ${H("M")}${H("<sub>xx,max</sub>")} = ${lt(U(c))}
  </div>
  <div class="eq">
...
<h2>8. Nodal Displacements ${x.length > 8 ? `(first 8 of ${x.length})` : ""}</h2>
<table>
...
`;
  ...
  W.derive(() => {
    ...
    const p = 0.3, b = (st.Ex.value.val + st.Ey.value.val) / 2 / (2 * (1 + p));
    ...
    }, Z.deformOutputs.val = Ot(t, a, Z.nodeInputs.val, Z.elementInputs.val), Z.analyzeOutputs.val = It(t, a, Z.elementInputs.val, Z.deformOutputs.val), (_a = Z.deformOutputs.val) == null ? void 0 : _a.deformations) {
      let h = 0;
  ...
  const et = W.state(""), dt = W.state(void 0);
  W.derive(() => {
    var _a;
    if (et.val === "Calcpad") try {
      const t = nt({
        template: $t,
        data: Z
      });
...
<p style="font-size:11px; color:#666;">Generated with Awatif - Plate FEM Analysis</p>
      ...
      b.href = v, b.download = "plate_fem_report.html", document.body.appendChild(b), b.click(), document.body.removeChild(b), URL.revokeObjectURL(v), et.val = "";
    }
  });
  document.body.append(Pt(st), At({
    mesh: Z,
    settingsObj: {
      nodes: false,
      deformedShape: true,
