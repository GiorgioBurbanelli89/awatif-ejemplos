...
import { s as re, B as C, o as ie, p as le, t as q, M as ae, u as ue, S as W, P as ce, E as F, y as j, z as L } from "./getReport-CMkXIj08.js";
let Ee, Me, Ae, Te, xe;
    ...
    for (let i in l) {
      ...
      u.get(Number(i)).forEach((p) => {
        ...
        const b = d[p].map((M) => t[M]), w = b.map((M, A) => A), B = (M) => {
          ...
          polygon: w,
          maxMeshSize: I
        }), H = n.length, z = c.length, N = R.map((M) => M.map((A) => A + H)), k = P.map((M, A) => A + H), $ = R.map((M, A) => A + z);
        n = [
          ...n,
          ...fe(P, y)
        ], c = [
        ...
        for (const M of v.keys()) {
          const A = v.get(M), S = Y(n, k, A, y);
          S !== null && r.get(Number(i)).set(M, S);
        }
        for (const M of T.keys()) {
          const A = T.get(M), S = Y(n, k, A, y);
