import { e as ve, f as xe, h as Te, i as se, x as B, v as n, g as we, j as Me, b as Ie, d as Se } from "./styles-pdtxawzU.js";
import { a as $e } from "./analyze-CmnHHflB.js";
import { d as Ae, __tla as __tla_0 } from "./deformCpp-CS-wCYO-.js";
import { g as Ee } from "./getParameters-Dl3mTgVW.js";
import { g as Oe } from "./getDialog-SKblNfmB.js";
import { g as V } from "./getReport-9byhZbfH.js";
...
Promise.all([
  ...
  n.derive(() => {
    ...
    if (s = t / Math.round(t / s), Math.abs(m - 0.5 * (c + v)) > 0.3 || Math.abs(l - 0.5 * (f + i)) > 0.3) {
      ...
      const a = Math.round((t - 2 * s) / s), u = y >= 2 && a >= 1, U = (c - m) / t, G = c - U * s, S = (f - l) / t, P = f - S * s, { nodes: A, elements: j, topNodesIndices: F, chordsIndices: ue, websIndices: me } = _(u ? t - s : t, s, r, u ? G : c, m, u ? P : f, l, u ? s : 0);
      h.push(...A), o.push(...j), M.push(...F), x.push(...ue), I.push(...me);
      const X = (m - v) / t, ee = (l - i) / t;
      let q = r;
      r === 1 && (q = 2), r === 2 && (q = 1);
      const { nodes: k, elements: fe, topNodesIndices: ge, chordsIndices: be, websIndices: ye } = _(u ? t - 2 * s : t - s, s, q, m - X * s, u ? v + X * s : v, l - ee * s, u ? i + ee * s : i, t + s);
      if (x.push(...Z(be, o.length)), I.push(...Z(ye, o.length)), o.push(...Le(fe, h.length)), M.push(...Z(ge, h.length)), h.push(...k), u) {
        h.push([
          0,
          0,
          y == 3 ? c + f : f
      ...
      if (r === 1 && (o.push([
        ...
        const O = A.length + k.length;
        w.push(O, O + 1);
      } else d === 1 ? w.push(0, A.length + k.length / 2 - 1) : w.push(A.length / 2, A.length + k.length - 1);
    } else {
      const a = Math.round((t - 2 * s) / s), u = y >= 2 && a >= 1, U = (c - v) / t, G = (f - i) / t, { nodes: S, elements: P, topNodesIndices: A, chordsIndices: j, websIndices: F } = _(u ? t - 2 * s : t, s, r, u ? c - U * s : c, u ? v + U * s : v, u ? f - G * s : f, u ? i + G * s : i, u ? s : 0);
