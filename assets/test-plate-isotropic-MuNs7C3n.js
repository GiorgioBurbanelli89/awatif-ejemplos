import { x as k, v as r, f as E, h as z, l as N, i as L, j as T, k as C } from "./getReport-CnIkx0ml.js";
import { a as P } from "./analyze-CmnHHflB.js";
import { d as G, __tla as __tla_0 } from "./deformCpp-CS-wCYO-.js";
import { g as S } from "./getParameters-Ce3duQQe.js";
import { g as R, __tla as __tla_1 } from "./getMesh-DmUdekin.js";
import { o as I } from "./unsafe-html-DSLeEmZR.js";
...
Promise.all([
  ...
  function M(l) {
    const t = (l == null ? void 0 : l.validation) || {}, d = t.w_fem ?? 0, c = t.w_navier ?? 0, v = t.error ?? 0, m = t.numNodes ?? 0, f = t.numElements ?? 0, s = t.params || {}, a = (w, D = 4) => Math.abs(w) < 1e-10 ? "0" : Math.abs(w) > 1e6 || Math.abs(w) < 1e-3 ? w.toExponential(D) : w.toFixed(D), e = s.E * Math.pow(s.t, 3) / (12 * (1 - s.nu * s.nu)), g = s.E / (2 * (1 + s.nu)), h = 5 / 6 * g * s.t, p = e, u = e * s.nu, x = e * (1 - s.nu) / 2, n = v < 10, o = `
<style>
  .validation-report {
...
<div class="validation-report">
  ...
  <div class="formula">
    ...
    <div class="eq"><var>D</var><sub>s</sub> = <var>\u03BA</var>\xB7<var>G</var>\xB7<var>t</var> =
      (5/6) \xB7 ${a(g / 1e9)} \xB7 ${a(s.t)} = <span class="result">${a(h)} N/m</span></div>
  ...
  <div class="matrix">
    ...
    <table>
      <tr><td>${a(h)}</td><td>0</td></tr>
      <tr><td>0</td><td>${a(h)}</td></tr>
    </table>
  ...
  <div class="mesh-info">
    ...
    <div><strong>Elementos</strong><br>${f}</div>
  ...
  <ul>
    ...
    <li><strong>Discretizaci\xF3n:</strong> Con ${f} elementos triangulares, error t\xEDpico ~${a(v, 1)}%.</li>
  </ul>
...
`;
  ...
  function _(l, t, d, c, v, m) {
    const f = c * Math.pow(d, 3) / (12 * (1 - v * v));
    let s = 0;
    for (let a = 1; a <= 21; a += 2) for (let e = 1; e <= 21; e += 2) {
      const g = a * Math.PI / l, h = e * Math.PI / t, p = a * e * Math.pow(Math.pow(g, 2) + Math.pow(h, 2), 2), u = 16 * m / (Math.pow(Math.PI, 6) * f * p), x = Math.pow(-1, (a + e) / 2 - 1);
      s += x * u;
    }
    return Math.abs(s);
  }
  ...
  r.derive(() => {
    var _a;
    const l = b.a.value.val, t = b.b.value.val, d = b.t.value.val, c = b.E.value.val, v = b.nu.value.val, m = b.q.value.val, f = b.meshDiv.value.val, s = Math.max(l, t) / f, { nodes: a, elements: e, boundaryIndices: g } = R({
      points: [
        [
          0,
          0,
    ...
    }, i.deformOutputs.val = G(a, e, i.nodeInputs.val, i.elementInputs.val), i.analyzeOutputs.val = P(a, e, i.elementInputs.val, i.deformOutputs.val);
    let p = 0;
