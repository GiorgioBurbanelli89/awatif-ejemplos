import { x as bt, v as Y, f as dt, h as At, l as Nt, i as Rt, j as Tt, k as It } from "./getReport-CnIkx0ml.js";
import { a as Ot } from "./analyze-CmnHHflB.js";
import { d as Pt, __tla as __tla_0 } from "./deformCpp-CS-wCYO-.js";
import { g as Lt } from "./getParameters-Ce3duQQe.js";
import { g as _t, __tla as __tla_1 } from "./getMesh-DmUdekin.js";
import { o as ut } from "./unsafe-html-DSLeEmZR.js";
...
`;
    for (let l = 0; l < t.length; l++) {
      ...
      for (let b = 0; b < t[l].length; b++) s += `<span class="td">${B(t[l][b], o)}</span>`;
...
<stop offset="100%" style="stop-color:#ff0000"/>
    ...
    for (let r = 0; r <= 5; r++) {
      ...
      w += `<text x="${_ + 20}" y="${T(g + 3, 0)}">${B(c, 4)}</text>`;
    }
...
<h4>Input data</h4>
<table><tr><td>
<p>Plate dimensions - ${j(`${y("B")} = ${B(k, 1)}`)} m, ${j(`${y("L")} = ${B(f, 1)}`)} m</p>
<p>Thickness - ${j(`${y("t")} = ${B(I, 3)}`)} m</p>
<p>Load - ${j(`${y("q")} = ${B(Math.abs(A), 1)}`)} kN/m\xB2</p>
<p>Modulus of elasticity - ${j(`${y("E")} = ${B(F, 0)}`)} MPa</p>
<p>Poisson's ratio - ${j(`${y("\u03BD")} = ${B(u, 2)}`)}</p>
...
<p>${j(`${y("G")} = ${et(y("E"), `2\xB7(1 + ${y("\u03BD")})`)} = ${et(B(F, 0), `2\xB7(1 + ${B(u, 2)})`)} = <b>${B(v, 2)}</b>`)} MPa</p>

<p><b>Flexural rigidity</b></p>
<p>${j(`${y("D")} = ${et(`${y("E")}\xB7${y("t")}<sup>3</sup>`, `12\xB7(1 - ${y("\u03BD")}<sup>2</sup>)`)} = ${et(`${B(F, 0)}\xB7${B(I, 3)}<sup>3</sup>`, `12\xB7(1 - ${B(u, 2)}<sup>2</sup>)`)} = <b>${B(C, 4)}</b>`)}</p>

<h4>Finite element mesh</h4>
<p>We use triangular finite elements with 18 DOFs (6 per node)</p>
<p>Total number of elements - ${j(`${y("n", "e")} = ${S}`)}</p>
<p>Total number of joints - ${j(`${y("n", "j")} = ${z}`)}</p>
<p>Supported joints count - ${j(`${y("n", "s")} = ${g}`)}</p>
...
<p>${j(`${y("N", "1")}(\u03BE,\u03B7) = 1 - \u03BE - \u03B7`)}</p>
<p>${j(`${y("N", "2")}(\u03BE,\u03B7) = \u03BE`)}</p>
<p>${j(`${y("N", "3")}(\u03BE,\u03B7) = \u03B7`)}</p>

<p><b>Constitutive matrix</b></p>
<p>${j(`<b>${y("D")}</b> = ${et(`${y("E")}\xB7${y("t")}<sup>3</sup>`, `12\xB7(1 - ${y("\u03BD")}<sup>2</sup>)`)} \xB7 <b class="b1">[</b>1; ${y("\u03BD")}; 0 <b class="b0">|</b> ${y("\u03BD")}; 1; 0 <b class="b0">|</b> 0; 0; ${et(`1 - ${y("\u03BD")}`, "2")}<b class="b1">]</b>`)}</p>
<p>${j(`<b>${y("D")}</b> = ${Bt(c, 4)}`)}</p>

<h4>Solution</h4>
<p>Total degrees of freedom: ${j(`${z * 6}`)}</p>
<p>Free degrees of freedom: ${j(`${z * 6 - g * 6}`)}</p>
<p>Global system: ${j(`${y("K")} \xB7 ${y("U")} = ${y("F")}`)}</p>

<h4>Results</h4>
<p><b>Displacements</b></p>
<p>Maximum displacement: ${j(`${y("w", "max")} = <span class="${Math.abs(P) > 1 ? "err" : "ok"}">${B(P, 6)}</span>`)} at joint ${$ + 1}</p>

${x.length > 0 ? `<p><b>Displacement contour map</b></p>${Xt(o, s, x, 400, "w")}` : ""}

<h4>Shell Results - Bending Moments</h4>
<p>Maximum ${y("M", "x")}: ${j(`${y("M", "x,max")} = ${B(w, 4)}`)} at element ${_ + 1}</p>
<p>Maximum ${y("M", "y")}: ${j(`${y("M", "y,max")} = ${B(G, 4)}`)} at element ${r + 1}</p>
...
<p>${j(`${y("w", "max")} \u2248 0,00406 \xB7 ${et(`|${y("q")}| \xB7 ${y("a")}<sup>4</sup>`, y("D"))}`)}</p>
<p>${j(`${y("w", "analytical")} = 0,00406 \xB7 ${et(`|${B(A, 1)}| \xB7 ${B(D, 1)}<sup>4</sup>`, B(C, 4))} = ${B(R, 6)}`)}</p>
<p>FEM result: ${j(`${y("w", "FEM")} = ${B(Math.abs(P), 6)}`)}</p>
<p>Error: ${j(`<span class="${W < 10 ? "ok" : "err"}">${B(W, 2)}%</span>`)}</p>
...
`;
  ...
  Y.derive(() => {
    ...
    }, V.deformOutputs.val = Pt(F, u, V.nodeInputs.val, V.elementInputs.val), V.analyzeOutputs.val = Ot(F, u, V.elementInputs.val, V.deformOutputs.val);
  });
