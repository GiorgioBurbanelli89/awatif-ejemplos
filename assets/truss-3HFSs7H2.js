import { e as C, f as I, k as D, h as x, x as b, i as M, v as o, g as H, j as P, b as F, d as _ } from "./styles-pdtxawzU.js";
...
import { g as G } from "./getDialog-SKblNfmB.js";
import { g as T } from "./getReport-9byhZbfH.js";
...
Promise.all([
  ...
  function A(a) {
    ...
    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 900px; margin: 0 auto; padding: 20px;">
      ...
      <p style="font-size: 18px; background: #fff0f0; padding: 15px; border-radius: 8px;">
        ${E(`${U("N")}<sub>max</sub> = <b style='color:#cc0000;'>${c(M(f), 2)} ${h}</b>`)}
        at Element ${$}
      </p>
  ...
  o.derive(() => {
    const a = g.span.value.val, e = g.divisions.value.val, u = g.height.value.val, m = g.elasticity.value.val * 1e6, p = g.area.value.val * 1e-4, l = g.load.value.val, h = [], r = [], v = a / e, f = [];
    for (let t = 0; t <= e; t++) {
      const i = [
        v * t,
        0,
...
${((_a = T({
      ...
      l.href = p, l.download = "truss_report.html", document.body.appendChild(l), l.click(), document.body.removeChild(l), URL.revokeObjectURL(p), y.val = "";
    }
  });
  document.body.append(B(g), H({
    mesh: w,
    settingsObj: {
      deformedShape: true
    }
